...
// Freeing a block then allocating the same size must reuse it.
int test_7() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
...
// verified before free. Catches overlapping blocks, header corruption
// and free-list corruption.
int test_12() {
  Allocator allocator = {0};
...
// Forward coalescing: free B, then free A. A must absorb B.
// Merged payload: 32 + 32 + 24 = 88.
int test_13() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
  void *a = alloc_malloc(&allocator, 32);
...
// Fragmentation regression: 9 blocks of 48, freed in scattered order,
// must merge back into one block of 9*48 + 8*24 = 624.
int test_17() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
  void *p[9];
...
// Explicit list: with LIFO insert at HEAD and first-fit from HEAD, the
// most recently freed fitting block must be handed out first.
int test_18() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
  void *a = alloc_malloc(&allocator, 32); // guards keep frees non-adjacent
  void *b = alloc_malloc(&allocator, 32);
  void *c = alloc_malloc(&allocator, 32);
  void *d = alloc_malloc(&allocator, 32);
  void *e = alloc_malloc(&allocator, 32);
  (void)a;
  (void)c;
  (void)e;
  alloc_free(&allocator, b);
  alloc_free(&allocator, d); // d freed last -> list head
  void *r = alloc_malloc(&allocator, 32);
  if (r != d) {
    printf("18. Most recently freed block not reused first "
           "(d=%p r=%p)\n",
           d, r);
    alloc_deinit(&allocator);
    return 1;
  }
  void *s = alloc_malloc(&allocator, 32);
  if (s != b) {
    printf("18. Remaining free block not found (b=%p s=%p)\n", b, s);
    alloc_deinit(&allocator);
    return 1;
  }
  alloc_deinit(&allocator);
  printf("Test 18 - Success\n");
  return 0;
}

// Free block too small for the request must be skipped (bump instead),
// but must stay in the list and serve a later fitting request.
int test_19() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
  void *a = alloc_malloc(&allocator, 32);
  void *b = alloc_malloc(&allocator, 32); // guard
  (void)b;
  alloc_free(&allocator, a);
  void *big = alloc_malloc(&allocator, 64); // does not fit in a's 32
  if (big == 0 || big == a) {
    printf("19. Too-small free block wrongly used for big request\n");
    alloc_deinit(&allocator);
    return 1;
  }
  void *small = alloc_malloc(&allocator, 32); // fits a exactly
  if (small != a) {
    printf("19. Skipped free block lost from the list (a=%p small=%p)\n", a,
           small);
    alloc_deinit(&allocator);
    return 1;
  }
  alloc_deinit(&allocator);
  printf("Test 19 - Success\n");
  return 0;
}

// Full lifecycle twice: free list state must not leak across
// deinit/init (dangling HEAD into an unmapped region).
int test_20() {
  for (int round = 0; round < 2; round++) {
    Allocator allocator = {0};
    if (alloc_init(&allocator, 4096) != 0) {
      printf("20. Init failed on round %d\n", round);
      return 1;
    }
    void *a = alloc_malloc(&allocator, 32);
    void *b = alloc_malloc(&allocator, 32);
    if (a == 0 || b == 0) {
      printf("20. Malloc failed on round %d\n", round);
      alloc_deinit(&allocator);
      return 1;
    }
    memset(a, 0x42, 32);
  ...
  printf("Test 20 - Success\n");
  return 0;
}

// Reset must also forget the free list: blocks freed before reset
// must not be handed out after it.
int test_21() {
  Allocator allocator = {0};
  alloc_init(&allocator, 4096);
  void *a = alloc_malloc(&allocator, 32);
  ...
  alloc_reset(&allocator);
  void *c = alloc_malloc(&allocator, 32);
  if (c != allocator.start_ptr + 2 * sizeof(size_t)) {
    printf("21. Malloc after reset did not start from the beginning\n");
    alloc_deinit(&allocator);
    return 1;
  }
  alloc_deinit(&allocator);
